...
1. The Controller creates a `BucketContent` object with its `BucketClassName` set to the name of its `BucketClass` and a `finalizer`.
...
1. `secretRef`: the name and namespace of the source secret in the `Bucket`'s namespace, as define in `bucketRef.namespace`.  This `secret` may be generated by the driver or written by an admin.
