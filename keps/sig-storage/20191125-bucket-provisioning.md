...
1. `azureBlobIdentity`: data required to target a provisioned azure container and/or storage account
1. `s3Identity`: data required to target a provisioned S3 bucket and/or user
1. `gcsIdentity`: data required to target a provisioned GCS bucket and/or service account
...
1. `bucketNameActual`: (Optional) Names the object store bucket for brownfield and static cases.
